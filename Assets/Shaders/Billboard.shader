    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert (appdata v)
            {
                ...
                // Getting the rotation matrix from the model matrix
                float3 rightM = normalize(m._m00_m01_m02);
                float3 upM = normalize(m._m10_m11_m12);
                float3 forwardM = normalize(m._m20_m21_m22);
                ...
                float4x4 modelRotationMatrixInverse = transpose(modelRotationMatrix);

                //apply the rotationMatrixInverse, model, view and projection matrix
                float4 pos = v.vertex;
                pos = mul(rotationMatrixInverse, pos);
                pos = mul(modelRotationMatrixInverse, pos);
